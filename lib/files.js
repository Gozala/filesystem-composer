...
, open: function open(path, options) {
    throw new Error('`open` is not implemented yet')
  }
   ...
   * @returns {Promise}
   */
, remove: Promised(function remove(path) {
   ...
   */
, read: Promised(function read(path, options) {
   ...
   * @returns {Promise << String}
   */
, write: Promised(function write(path, data, options) {
   ...
   * @returns {Promise}
   */
, copy: Promised(function copy(source, target) {
    Array.prototype.splice.call(arguments, 1, 1, this.read(source))
    this.write(target, this.read(source))
  })
   ...
   * @returns {Promise}
   */
, rename: Promised(function rename(source, name) {
   ...
   * @returns {Promise}
   */
, move: Promised(function move(source, target) {
    this._move.apply(this, arguments)
  })
