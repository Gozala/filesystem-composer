...
function existingPath(fs, path) {
  return when(fs.exists(path), function(exists) {
    return exists ? fs.Path(path) : existingPath(fs, fs.directory(path))
  })
}
function createPath(path, parts) {
  if (!parts.length) return true
  path = path.join(parts.shift())
  return when(path.makeDirectory(), function directoryCreated() {
    createPath(path, parts)
  })
}
...
, makeTree: function makeTree(path) {
    return when(existingPath(this, path), function existingPathFound(existing) {
      return createPath(existing, existing.relative(path).split())
    })
  }
...
, removeTree: function removeTree(path) {
    return when(this.list(path), function(entries) {
      ...
      var removeEntries = all(entries.map(function(entry) {
        ...
        , function removed() { return true }
          // If remove was rejected then promise was not a file so
          // trying to remove tree in that case.
        , function notFile() { return this.removeTree(entryPath) }.bind(this)
        )
      }, this))
