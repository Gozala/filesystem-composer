...
*/
exports.compose = function compose(PATH) {
    // Array shortcuts
    ...
    /**
        ...
        @returns {Boolean}
    */
    function _isDrive(path) { return path.indexOf(':') === --path.length }
    /**
    ...
    */    
    function _isAbsolute(path) {
        var parts = split(path), head = parts[0]
        return (parts.length == 0) ? false : '' === head || _isDrive(head)
    }
    ...
    /**
        ...
        @returns {String()}
    */
    var join = PATH.join || (PATH.join = function join() {
        // special case for root, helps glob [""] -> "/"
        if (1 === arguments.length && '' === arguments[0]) return SEPARATOR
        // ["", ""] -> "/",
        // ["", "a"] -> "/a"
        // ["a"] -> "a"
    ...
    /**
        ...
        @return {String[]}
    */
    var split = PATH.split || (PATH.split = function split(path) {
        path = String(path)
    ...
    /**
        ...
        @return {String[]}
    */
    var resolve = PATH.resolve || (PATH.resolve = function resolve() {
    ...
    */
    var normal = PATH.normal = resolve
    /**
    ...
    */
    var directory = PATH.directory || (
        PATH.directory = function directory(path) {
    ...
    /**
        ...
        @returns {String}
    */
    var base = PATH.base || (PATH.base = function base(path, extension) {
        var basename = split(path).pop() || ''
        if (undefined !== extension) {
            var l = basename.length - extension.length
            if (extension === basename.substr(l)) 
    ...
    */
    ...
    /**
        ...
        @returns {String}
    */
    var absolute = PATH.absolute || (PATH.absolute = function absolute(path) {
        return resolve(join(PATH.workingDirectory(), ''), path)
    })
    /*
    ...
    */
    var canonical = PATH.canonical || (
        PATH.canonical = function canonical(path) {
            var paths = [PATH.workingDirectory(), path], outs = [], prev
            for (var i = 0, ii = paths.length; i < ii; i++) {
                var path = paths[i], ins = split(path)
                if (_isDrive(ins[0])) outs = [ins.shift()]
                while (ins.length) {
                    var leaf = ins.shift(),
                        consider = join.apply(
                            undefined, outs.concat([leaf])
                        )
    ...
    */
    var relative = PATH.relative || (
        PATH.relative = function relative(source, target) {
            if (!target) {
                target = source
                source = PATH.workingDirectory() + SEPARATOR
            }
    ...
    */
    var Path = PATH.path = PATH.Path || (PATH.Path = function Path(paths) {
        if (!(this instanceof Path) || 1 !== arguments.length) 
            return new Path(_slice.call(arguments))
        var path = ('string' === typeof paths) ? paths
            : (1 === paths.length && '' === paths[0]) ? ''
            : join.apply(null, paths) || '.'
        this.toString = function toString() { return path }
    })
    var PathProto = Path.prototype
    if (!(PathProto instanceof String))
        PathProto = Path.prototype = new String()
    PathProto.constructor = Path
    PathProto.valueOf = function valueOf() { return this.toString() }
    PathProto.absolute = function() {
        return new this.constructor(absolute(this.toString()))
    }
    PathProto.join = function() {
        var rest = _slice.call(arguments)
        rest.unshift(this.toString())
        return new this.constructor(join.apply(null, rest))
    }
    PathProto.resolve = function() {
        var rest = _slice.call(arguments)
        rest.unshift(this.toString())
        return new this.constructor(resolve.apply(null, rest))
    }
    PathProto.to = function to(target) {
        return new this.constructor(relative(this.toString(), target))
    }
    PathProto.from = function from(target) {
        return new this.constructor(relative(target, this.toString()))
    }
    PathProto.base = function(extension) {
        return new this.constructor(base(this.toString(), extension))
    }
    PathProto.canonical = function(path) {
        return new this.constructor(canonical(this.toString(), path))
    }
    PathProto.directory = function() {
        return new this.constructor(directory(this.toString()))
    }
    PathProto.normal = function() {
        return new this.constructor(normal(this.toString()))
    }
    PathProto.relative = function(target) {
        return new this.constructor(relative(this.toString(), target))
    }
    return PATH
}