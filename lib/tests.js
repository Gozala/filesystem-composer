...
, isDirectory: function isDirectory(path) {
    return when(this.isFile(path), function(isFile) { return !isFile })
  }
...
, isLink: function isLink(path) {
    throw new Error('`isLink` is not implemented yet')
  }
  /**
   * @param {String(Promise|Path)|String} path
   * @returns {Promise << Boolean}
...
, same: function same(source, target) {
    throw new Error('`same` is not implemented yet')
  }
...
, sameFilesystem: function sameFilesystem(source, target) {
    throw new Error('`sameFilesystem` is not implemented yet')
  }
})
